 ...
 * @author Chethiya
 *
 * Holds the configs reuired to configure the two-way ssl enabled RestTemplate.
 */
@Slf4j
@Configuration
public class HelloConfig {
    ...
    public RestTemplate getTwoWaySSLRestTemplate() {
        ...
        try {
            log.debug("Initializing the keystore, {}", securityProperties.getKeystoreName());
            ...
            log.debug("Initializing ssl context for, {}", securityProperties.getKeystoreName());
            ...
            log.error("Error during creating the two way SSL enabled restTemplate ", exception);
        }
        return restTemplate;
    }
}
